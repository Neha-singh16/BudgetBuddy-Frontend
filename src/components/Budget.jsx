...
export default function Budget() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 p-3 sm:p-4 md:p-6 relative overflow-hidden">
      <FloatingCoins />
        ...
        >
          <div className="flex items-start sm:items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
            <motion.div
              ...
              className="w-14 sm:w-16 md:w-20 h-14 sm:h-16 md:h-20 rounded-lg sm:rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-500 flex items-center justify-center shadow-lg flex-shrink-0"
            >
              <PiggyBank className="text-white w-7 sm:w-8 md:w-10" />
            </motion.div>
            <div>
              <h1 className="text-xl sm:text-3xl md:text-4xl lg:text-5xl font-black bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 bg-clip-text text-transparent flex items-center gap-2 sm:gap-3">
                Budget
                <motion.div
                  animate={{ rotate: 360 }}
                  transition={{ duration: 2, repeat: Infinity, ease: 'linear' }}
                >
                  <Sparkles className="text-emerald-500 w-5 sm:w-6 md:w-7" />
                </motion.div>
              </h1>
              <p className="text-xs sm:text-sm md:text-base text-gray-600 font-medium">Create and track spending limits</p>
            </div>
          </div>
        </motion.div>

        {/* Stats Cards - Mobile-First (Single Column) */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 sm:gap-5 md:gap-6 mb-8">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.1 }}
            whileHover={{ y: -5, boxShadow: '0 20px 40px rgba(16, 185, 129, 0.3)' }}
            className="bg-white/70 backdrop-blur-xl rounded-lg sm:rounded-2xl p-4 sm:p-5 md:p-6 border border-emerald-200/50 shadow-md sm:shadow-lg relative overflow-hidden cursor-pointer"
            onClick={() => { setFilterPeriod('all'); scrollToList(); }}
          >
            <div className="absolute top-0 right-0 w-24 sm:w-32 h-24 sm:h-32 bg-gradient-to-br from-emerald-500/20 to-transparent rounded-full blur-2xl" />
            <div className="relative">
              <div className="flex items-center justify-between mb-3 sm:mb-4">
                <div className="w-11 sm:w-12 md:w-14 h-11 sm:h-12 md:h-14 rounded-lg sm:rounded-xl bg-gradient-to-br from-emerald-500 to-teal-500 flex items-center justify-center flex-shrink-0">
                  <Wallet className="text-white w-5 sm:w-6 md:w-7" />
                </div>
                <motion.div
                  animate={{ scale: [1, 1.2, 1] }}
                  transition={{ duration: 2, repeat: Infinity }}
                >
                  <Sparkles className="text-emerald-500 w-4 sm:w-5 md:w-6" />
                </motion.div>
              </div>
